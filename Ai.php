...
/**
 * Last.php — VOID MASTER vNext (Real-Time + Trap Detector + Auto-Learning Brain)
 * Single-file PHP (PHP 7.4+) — SQLite backed EWMA & History & League Profiles
 *
 * Endpoints:
 *  - ?action=master_analyze (POST JSON)  -> full engine analyze (manual / triggered)
 *  - ?action=void_analyze   (POST JSON)  -> quick compatibility
 *  - ?action=ewma_stats     (GET)        -> EWMA table
 *  - ?action=profile_stats  (GET)        -> League profiles
 *
 * Notes:
 *  - This file requires PHP PDO SQLite extension enabled.
 *  - Client-side optional real-time polling at 200ms (toggleable).
 */
...
function adaptive_alpha_tune(string $k) {
    ...
    // alpha higher when volatility higher (faster adapt)
    $newAlpha = 0.02 + ($norm * 0.78);
    ewma_set_alpha($k, $newAlpha);
}

// ---------------- LEAGUE PROFILE (Auto-Learning Brain) ----------------
function update_league_profile(string $league, float $move) {
    global $pdo;
    if (!$pdo) return;
    $st = $pdo->prepare("SELECT sample_count,avg_move,volatility FROM league_profiles WHERE league=:league");
    $st->execute([':league'=>$league]);
    $row = $st->fetch(PDO::FETCH_ASSOC);
    $ts = time();
    if (!$row) {
        $ins = $pdo->prepare("INSERT INTO league_profiles (league,sample_count,avg_move,volatility,updated_at) VALUES(:league,:n,:avg,:vol,:t)");
        $ins->execute([':league'=>$league,':n'=>1,':avg'=>$move,':vol'=>0.0,':t'=>$ts]);
        return;
    }
    $n = intval($row['sample_count']) + 1;
    $prevAvg = floatval($row['avg_move']);
    $newAvg = (($prevAvg * ($n-1)) + $move)/$n;
    // volatility as rolling std approx
    $prevVol = floatval($row['volatility']);
    $newVol = (($prevVol * ($n-1)) + abs($move - $newAvg)) / $n;
    $upd = $pdo->prepare("UPDATE league_profiles SET sample_count=:n, avg_move=:avg, volatility=:vol, updated_at=:t WHERE league=:league");
    $upd->execute([':n'=>$n,':avg'=>$newAvg,':vol'=>$newVol,':t'=>$ts,':league'=>$league]);
}

// ---------------- GET PROFILE ----------------
function get_league_profile(string $league) {
    global $pdo;
    if (!$pdo) return null;
    $st = $pdo->prepare("SELECT * FROM league_profiles WHERE league=:league");
    $st->execute([':league'=>$league]);
    $r = $st->fetch(PDO::FETCH_ASSOC);
    return $r ? $r : null;
}

// ---------------- LVE / SFD / OUCE / SMK / FDA / LTME / SFR / VPE / HPC / CER / ESP / Value Spotter ----------------
// (Take the functions from previous merged version — kept concise here for readability)
...
function mmra_engine(array $input): array {
    ...
    $rows = [];
    if ($pdo) {
        $st = $pdo->prepare("SELECT sample,ts FROM samples WHERE k='master_netflow' ORDER BY id DESC LIMIT 32");
        $st->execute();
        $rows = $st->fetchAll(PDO::FETCH_ASSOC);
    }
    $vals = array_map(function($r){ return floatval($r['sample']); }, $rows);
...
// ---------------- ADVANCED TRAP DETECTOR ----------------
function detect_trap_advanced(array $ah_details, array $flow1, array $historySamples = []): array {
    // Advanced heuristics to detect traps: liquidity squeeze, bait-and-switch, staged reversal
    $trap_score = 0.0; $reasons = [];
    // 1) AH moves strong but 1x2 calm
    foreach ($ah_details as $ad) {
        $hMove = abs(($ad['open_home'] ?? 0) - ($ad['now_home'] ?? 0));
        $aMove = abs(($ad['open_away'] ?? 0) - ($ad['now_away'] ?? 0));
        if (($hMove > 0.10 && abs($flow1['home'] ?? 0) < 0.03)) { $trap_score += 0.25; $reasons[]='AH strong move but 1x2 calm (home)'; }
        if (($aMove > 0.10 && abs($flow1['away'] ?? 0) < 0.03)) { $trap_score += 0.25; $reasons[]='AH strong move but 1x2 calm (away)'; }
    }
    // 2) rapid bounce patterns in recent samples
    if (!empty($historySamples)) {
        $bounceCount=0; $last = null;
        foreach ($historySamples as $s) {
            if ($last !== null && ($s * $last) < 0 && abs($s - $last) > 0.05) $bounceCount++;
            $last = $s;
        }
        if ($bounceCount >= 2) { $trap_score += 0.22; $reasons[]='Multiple quick bounces detected'; }
    }
    // 3) spoof signature: big spike then revert quickly
    $spike_detected=false;
    if (count($historySamples) >= 4) {
        $maxV = max($historySamples); $minV = min($historySamples);
        if (($maxV - $minV) > 0.3) { $spike_detected=true; $trap_score += 0.18; $reasons[]='Spike followed by revert pattern'; }
    }
    // 4) CMF risk cross-check (if provided in AH details meta)
    // Normalize
    $trap_score = clampf($trap_score, 0.0, 1.0);
    return ['trap_score'=>$trap_score,'reasons'=>array_values(array_unique($reasons))];
}
...
function master_analyze(array $payload): array {
    ...
    // persist EWMA & samples (note: only on analyze actions)
    ...
    // history samples to detect bounces
    $histRows = get_last_samples('master_netflow', 16);
    $histVals = array_map(function($r){ return floatval($r['sample']); }, $histRows);
    $trap_adv = detect_trap_advanced($ah_details, $flow1, $histVals);
    // update league profile
    update_league_profile($league, abs($agg_sample));
    // final labeling & signature
    if ($market_kill) { $final_label='💀 MARKET KILL — ห้ามเสี่ยง'; $recommendation='ห้ามเสี่ยง/จำกัดเดิมพัน'; }
    elseif ($void_score > 0.35 && $smai['smai_verdict'] !== 'Likely_Trap') { $final_label='✅ ไหลจริง — ฝั่งเหย้า'; $recommendation='พิจารณาตาม (ควบคุมความเสี่ยง)'; }
    elseif ($void_score < -0.35 && $smai['smai_verdict'] !== 'Likely_Trap') { $final_label='✅ ไหลจริง — ฝั่งเยือน'; $recommendation='พิจารณาตาม (ควบคุมความเสี่ยง)'; }
    elseif ($trap || $trap_adv['trap_score'] > 0.35 || $smai['smai_verdict'] === 'Likely_Trap') { $final_label='❌ พบกับดัก/หลอก'; $recommendation='ไม่แนะนำเดิมพัน'; }
    else { $final_label='⚠️ สัญญาณผสม — รอ confirm'; $recommendation='รอดูการยืนยัน'; }
    return [
        'status'=>'ok','input'=>['home'=>$home,'away'=>$away,'favorite'=>$favorite,'league'=>$league,'open1'=>$open1,'now1'=>$now1,'ah'=>$ah_list,'context'=>$context],
        'metrics'=>array_merge($metrics_for_void,['marketMomentum'=>$marketMomentum,'total1x2_mom'=>$total1x2_mom,'totalAH_mom'=>$totalAH_mom,'reboundSens'=>$reboundSens,'flowPower'=>$flowPower,'confidence'=>$confidence,'signature'=>$signature,'trapFlags'=>$trapFlags,'fda'=>$fda,'smk_flags'=>$smartFlags,'lve'=>$lve,'sfd'=>$sfd,'ouce'=>$ouce,'ltme'=>$ltme,'vpe'=>$vpe,'sfr'=>$sfr,'cmf'=>$cmf,'mmra'=>$mmra,'smai'=>$smai,'trap_adv'=>$trap_adv]),
        'void_score'=>$void_score,'final_label'=>$final_label,'recommendation'=>$recommendation,'predicted_winner'=>$predicted_winner,
        'hackScore'=>$hackScore,'true_prob'=>$true_prob,'market_prob_now'=>$market_imp_now,'market_prob_open'=>$market_imp_open,'esp'=>$esp,'value'=>$value,'ah_details'=>$ah_details,'cmf'=>$cmf,'mmra'=>$mmra,'smai'=>$smai,'trap_adv'=>$trap_adv
    ];
}

// ---------------- compatibility quick analyze ----------------
function void_analyze(array $payload): array { return master_analyze($payload); }
...
if (php_sapi_name() !== 'cli') {
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'profile_stats') {
        header('Content-Type: application/json; charset=utf-8');
        if (!$pdo) { echo json_encode(['status'=>'error','msg'=>'no_db']); exit; }
        $st = $pdo->query("SELECT * FROM league_profiles");
        $data = $st->fetchAll(PDO::FETCH_ASSOC);
        echo json_encode(['status'=>'ok','profiles'=>$data], JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT); exit;
    }
}

// ---------------- FRONTEND UI (Real-time toggle + Dragon effects) ----------------
?><!doctype html>
<html lang="th">
<head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>LAST — VOID MASTER vNext</title>
<style>
...
.dragon{position:absolute;right:-80px;top:-60px;width:420px;height:220px;pointer-events:none;opacity:0.95}
.ink{position:absolute;left:0;bottom:0;width:100%;height:160px;pointer-events:none;opacity:0.14}
...
<div class="container">
  <div class="header">
    <div style="display:flex;gap:12px;align-items:center"><div class="logo">鴻</div><div><h1 style="margin:0;color:var(--gold)">LAST — VOID MASTER vNext</h1><div class="small">Real-Time 0.2s • Trap Detector • Auto-Learning Brain • Dragon UI</div></div></div>
    <div>
      <label class="toggle"><input id="rtToggle" type="checkbox"> Real-Time (200ms)</label>
    </div>
  </div>
  ...
  <div class="card">
    <form id="mainForm" onsubmit="return false;">
      ...
      <div class="card">
        <strong class="small">ลีก (ช่วยให้ระบบเรียนรู้พฤติกรรม)</strong>
        <input id="league" placeholder="เช่น EPL, LaLiga, Thai"/>
  ...
  <div id="resultWrap" class="card" style="display:none;position:relative;margin-top:14px">
    <svg class="dragon" viewBox="0 0 800 200" preserveAspectRatio="xMidYMid meet"><defs><linearGradient id="g1"><stop offset="0%" stop-color="#ffd78c"/><stop offset="100%" stop-color="#f59e0b"/></linearGradient><path id="p" d="M20,160 C150,10 350,10 480,140 C560,210 760,120 780,60"/></defs><use xlink:href="#p" fill="none" stroke="rgba(212,160,23,0.06)" stroke-width="8"/><circle r="14" fill="url(#g1)"><animateMotion dur="7s" repeatCount="indefinite"><mpath xlink:href="#p"></mpath></animateMotion></circle></svg>
  ...
  <div class="card small" style="margin-top:12px"><strong>หมายเหตุ</strong><div class="small">ระบบวิเคราะห์จากราคาที่ผู้ใช้กรอกเท่านั้น — EWMA learning เก็บบน server (SQLite). Real-Time polling จะไม่เขียน DB หากไม่ได้กด Analyze (ป้องกันการเขียนข้อมูลบ่อย)</div></div>
</div>
...
async function realtimeTick(){ // do light-weight analyze (no DB writes on server unless user clicks analyze)
  // We'll call master_analyze but server writes only when analyze invoked; here we'll pass flag realtime:true to get speed-optimized response (server currently doesn't branch; keep lightweight)
  const payload=collectPayload(); payload.realtime = true;
  try{
    ...
    // render small badges
    if (j && j.void_score !== undefined) {
      document.getElementById('voidScore').innerText = j.void_score.toFixed(4);
      document.getElementById('confidence').innerText = (j.metrics && j.metrics.confidence!==undefined) ? j.metrics.confidence + '%' : '--%';
      // minor visual: dragon flare when confidence>85
      if (j.metrics && j.metrics.confidence>85) flashDragon();
    }
  }catch(e){
    // ignore in realtime to avoid spamming UI errors
  }
}

function startRealtime(){ if(rtInterval) clearInterval(rtInterval); rtInterval = setInterval(realtimeTick, 200); realtimeTick(); }
function stopRealtime(){ if(rtInterval) clearInterval(rtInterval); rtInterval=null; }
...
function renderResult(r){ document.getElementById('voidScore').innerText = (r.void_score!==undefined)?r.void_score.toFixed(4):'--'; document.getElementById('confidence').innerText = r.metrics && r.metrics.confidence!==undefined ? (r.metrics.confidence+'%') : '--%'; document.getElementById('vpeHome').innerText = r.metrics && r.metrics.vpe ? r.metrics.vpe.home : '--'; document.getElementById('vpeAway').innerText = r.metrics && r.metrics.vpe ? r.metrics.vpe.away : '--';
  ...
  if (r.trap_adv) dhtml += `<div class="small" style="margin-top:8px">Advanced Trap: Score ${ (r.trap_adv.trap_score*100).toFixed(1) }% — ${ (r.trap_adv.reasons||[]).join('; ') }</div>`;
  dt.innerHTML=dhtml;
  const tome=document.getElementById('tome'); let t=`<h3 style="margin:0;color:var(--gold)">คัมภีร์ลับ</h3><div style="margin-top:8px">VoidScore:${r.void_score.toFixed(4)} Confidence:${r.metrics.confidence}% FlowPower:${r.metrics.flowPower}</div>`; t+=`<div style="margin-top:8px"><strong>CMF:</strong> ${JSON.stringify(r.cmf)}</div>`; t+=`<div style="margin-top:8px"><strong>SMAI:</strong> ${r.smai.smai_verdict} (${r.smai.smai_confidence}%)</div>`; t+=`<div style="margin-top:8px"><strong>Advanced Trap:</strong> ${(r.trap_adv && r.trap_adv.trap_score)?(r.trap_adv.trap_score*100).toFixed(1)+'%':''}</div>`; t+=`<div style="margin-top:8px"><strong>Recommendation:</strong> ${r.recommendation}</div>`; tome.innerHTML=t;
...
// ---------------- minor visuals ----------------
function flashDragon(){ const d=document.querySelector('.dragon'); if(!d) return; d.style.transform='scale(1.02)'; setTimeout(()=>{ d.style.transform='scale(1)'; },300); }
...
inputs.forEach(i => i.addEventListener('touchstart', () => { document.body.style.zoom = 1; }));
