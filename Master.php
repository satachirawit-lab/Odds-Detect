...
/**
 * Last_void_master.php
 * Single-file — Football Master Engine (merged / "ฉีกตลาด" edition)
 * PHP 7.4+, PDO SQLite
 *
 * Endpoints:
 *  - ?action=master_analyze  (POST JSON)  -> full engine
 *  - ?action=void_analyze    (POST JSON)  -> compatibility / quick analyze
 *  - ?action=ewma_stats      (GET)        -> EWMA table
 *
 * UI: Chinese ancient theme, responsive
 */
...
// ---------------- OUCE (OU/cross-market mismatch infer) ----------------
...
function master_analyze(array $payload): array {
    // core: combine all modules and return structured JSON
    ...
    // true probability (base model + void bias + vpe + hpc + cer)
    ...
    return [
        ...
        'hackScore'=>$hackScore,'true_prob'=>$true_prob,'market_prob_now'=>$market_imp_now,'market_prob_open'=>$market_imp_open,'esp'=>$esp,'value'=>$value,'ah_details'=>$ah_details
    ];
}

// ---------------- compatibility void_analyze (quick) ----------------
function void_analyze(array $payload): array {
    // keep minimal fallback to master_analyze
...
<div class="container">
  <div class="header">
    <div style="display:flex;gap:12px;align-items:center"><div class="logo">鴻</div><div><h1 style="margin:0;color:var(--gold)">LAST — VOID MASTER (Merged)</h1><div class="small">All engines integrated — LTME / SFR / VPE / SMK / FDA / LVE / SFD / HPC / CER / ESP</div></div></div>
    ...
    <div style="display:flex;gap:14px">
      ...
      <div style="width:340px;padding:12px">
        <div style="text-align:center">
          <div style="font-weight:900;font-size:1.4rem;color:var(--gold)" id="voidScore">--</div><div class="small">VOID Score</div>
          <div style="height:8px"></div>
          <div style="font-weight:900;font-size:1.2rem" id="confidence">--%</div><div class="small">Confidence</div>
          <div style="height:8px"></div>
...
function renderResult(r){ document.getElementById('voidScore').innerText = (r.void_score!==undefined)?r.void_score.toFixed(4):'--'; document.getElementById('confidence').innerText = r.metrics && r.metrics.confidence!==undefined ? (r.metrics.confidence+'%') : '--%'; document.getElementById('vpeHome').innerText = r.metrics && r.metrics.vpe ? r.metrics.vpe.home : '--'; document.getElementById('vpeAway').innerText = r.metrics && r.metrics.vpe ? r.metrics.vpe.away : '--';
  ...
  dt.innerHTML=dhtml;
  const tome=document.getElementById('tome'); let t=`<h3 style="margin:0;color:var(--gold)">คัมภีร์ลับ</h3><div style="margin-top:8px">VoidScore:${r.void_score.toFixed(4)} Confidence:${r.metrics.confidence}% FlowPower:${r.metrics.flowPower}</div>`; t+=`<div style="margin-top:8px"><strong>Value:</strong> ${JSON.stringify(r.value.labels)}</div>`; t+=`<div style="margin-top:8px"><strong>ESP:</strong> ${r.esp.expected_score}</div>`; t+=`<div style="margin-top:8px"><strong>Recommendation:</strong> ${r.recommendation}</div>`; tome.innerHTML=t;
