...
// index.php — Hong Ya Dong Ultimate — Ultra-Hard Engine + Smart Money Killer
// Single-file: UI + PHP analyzer. PHP 7.4+
// - Analyzes only from prices user provides (Open vs Now for 1x2 and AH lines)
// - Auto-Rebound Sensitivity computed from provided prices (no history/snapshots)
// - Ultra-hard scoring engine + Smart Money Killer detector
// - Visual Effects: dragon (SVG), smoke (CSS), ink splatter (canvas)
// Safety
...
// Auto-Rebound Sensitivity computed from single bar inputs (Open vs Now)
...
if (isset($_GET['action']) && $_GET['action'] === 'analyze' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ---------------- Ultra-Hard Engine + Smart Money Killer ----------------
    // 1) normalize motions and compute z-scores
    $motions = [];
    foreach (['home','draw','away'] as $s) {
    ...
    // 7) smart money killer detector (extra hardcore)
    $smartMoneyScore = 0.0; $smartFlags = [];
    // conditions that indicate smart money: high juice pressure, high concentration, lines stacked, low overround_now (tight book), high top momentum
    $overround_open = 0.0; $overround_now = 0.0; foreach (['home','draw','away'] as $s) { $io=impliedProb(safeFloat($open1[$s]??null)); $in=impliedProb(safeFloat($now1[$s]??null)); if(!is_nan($io)) $overround_open += $io; if(!is_nan($in)) $overround_now += $in; } $overround_open -=1.0; $overround_now -=1.0;
    ...
    $smart_money_killer = ($smartMoneyScore >= 0.7 && $flow1['home']!==0.0 || $smartMoneyScore>=0.7 && $flow1['away']!==0.0) ? true : false;

    // 8) aggregate raw signal
    ...
    // 12) final label and recommendation
    if ($market_kill) {
...
<title>หงหยาต้ง — Imperial Odds Analyzer (Smart Money Killer)</title>
<style>
:root{ --gold:#d4a017; --royal:#5b21b6; --card:#0f0b0a; --muted:#d9c89a; --danger:#ff3b30; }
*{box-sizing:border-box}
body{margin:0;font-family:Inter, 'Noto Serif TC', 'Noto Sans Thai', system-ui,-apple-system,'Segoe UI',Roboto,Arial;background:linear-gradient(180deg,#0b0810,#120916);color:#f6eedf}
...
.smoke{position:absolute;left:10px;top:10px;width:200px;height:200px;pointer-events:none;opacity:0.12;filter:blur(8px);animation:swirl 8s linear infinite}
...
<body>
  <div class="container">
    <div class="header">
      <div style="display:flex;align-items:center;gap:12px"><div class="logo">鴻</div><div><h1 style="margin:0;color:var(--gold)">ทีเด็ดเฮียเจ็ก</h1><div style="color:#d9c89a;font-size:0.9rem">ระบบโหดสุด — วิเคราะห์จากราคาที่ผู้ใช้กรอก</div></div></div>
    ...
    <div class="card">
      <form id="mainForm" onsubmit="return false;">
        ...
        <div class="card" style="padding:12px;background:linear-gradient(180deg,rgba(255,255,255,0.02),rgba(255,255,255,0.01))">
    ...
    <div id="resultWrap" class="card" style="display:none;position:relative;overflow:visible">
      <svg class="dragon" viewBox="0 0 800 200" preserveAspectRatio="xMidYMid meet">
        <defs>
          <linearGradient id="g1" x1="0%" x2="100%"><stop offset="0%" stop-color="#ffd78c"/><stop offset="100%" stop-color="#f59e0b"/></linearGradient>
          <path id="dragonPath" d="M20,160 C150,10 350,10 480,140 C560,210 760,120 780,60"/>
        </defs>
        <use xlink:href="#dragonPath" fill="none" stroke="rgba(212,160,23,0.06)" stroke-width="8"/>
        <circle r="14" fill="url(#g1)">
          <animateMotion dur="6s" repeatCount="indefinite"><mpath xlink:href="#dragonPath"></mpath></animateMotion>
        </circle>
      </svg>
    ...
    <div class="card small" style="margin-top:12px"><strong>หมายเหตุ</strong><div class="small">ระบบนี้วิเคราะห์จากราคาที่ผู้ใช้กรอกเท่านั้น — Auto-Rebound คำนวณสดจาก Open/Now ที่กรอก ไม่เก็บ Snapshot</div></div>
  </div>
...
async function analyze(){ const payload=collectPayload(); document.getElementById('resultWrap').style.display='block'; document.getElementById('mainSummary').innerHTML='<div class="small">กำลังวิเคราะห์…</div>'; try{ const res=await fetch('?action=analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const text=await res.text(); let json; try{ json=JSON.parse(text); }catch(e){ document.getElementById('mainSummary').innerHTML='<div class="small">Response parse error: '+e.message+'</div><pre style="background:#fff;padding:8px;border-radius:8px;margin-top:8px;color:#000">'+text.replace(/</g,'&lt;')+'</pre>'; return; } renderResult(json); spawnInk(); }catch(err){ document.getElementById('mainSummary').innerHTML='<div class="small">Fetch error: '+err.message+'</div>'; } }

document.getElementById('analyzeBtn').addEventListener('click', analyze);

function renderResult(r){ document.getElementById('reboundDisplay').value = r.metrics && r.metrics.reboundSens ? r.metrics.reboundSens : 'Auto'; document.getElementById('confValue').innerText = r.metrics && r.metrics.confidence!==undefined ? (r.metrics.confidence+'%') : '--%'; document.getElementById('flowPowerValue').innerText = r.metrics && r.metrics.flowPower!==undefined ? (r.metrics.flowPower) : '--'; document.getElementById('nfiValue').innerText = r.metrics && r.metrics.nfi!==undefined ? nf(r.metrics.nfi,4) : '--'; const mainSummary=document.getElementById('mainSummary'); let html=`<div style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-weight:900;font-size:1.15rem;color:var(--gold)">${r.final_label}</div><div style="margin-top:8px"><strong>คำแนะนำ:</strong> ${r.recommendation}</div><div style="margin-top:6px"><strong>คาดการณ์ทีมผู้ชนะ:</strong> <strong>${r.predicted_winner}</strong></div></div><div style="text-align:right"><div class="tag">Hack200%</div></div></div>`; mainSummary.innerHTML=html; if (r.metrics && r.metrics.market_kill) { mainSummary.insertAdjacentHTML('afterbegin','<div class="alarm pulse">⚠️ MARKET KILL ALERT — ห้ามเสี่ยง *</div>'); }
  ...
  const mainReasons=document.getElementById('mainReasons'); let reasonsHtml='<strong>เหตุผลโดยย่อ</strong>'; if(!r.reasons||r.reasons.length===0) reasonsHtml+='<div class="small" style="color:#cdb68e">ไม่พบสัญญาณพิเศษ</div>'; else { reasonsHtml+='<ul>'; r.reasons.forEach(rr=> reasonsHtml+='<li>'+rr+'</li>'); reasonsHtml+='</ul>'; } mainReasons.innerHTML=reasonsHtml; let dt=document.getElementById('detailTables'); let html2='<strong>รายละเอียด 1X2</strong>'; html2+='<table class="table"><thead><tr><th>ฝั่ง</th><th>Open</th><th>Now</th><th>NetFlow</th><th>Mom</th></tr></thead><tbody>'; ['home','draw','away'].forEach(side=>{ const o=r.input&&r.input.open1?r.input.open1[side]:'-'; const n=r.input&&r.input.now1?r.input.now1[side]:'-'; const net=r.flows_1x2&&r.flows_1x2[side]!==undefined?nf(r.flows_1x2[side],4):'-'; const mom=r.mom1&&r.mom1[side]!==undefined?nf(r.mom1[side],4):'-'; html2+=`<tr><td style="padding:6px">${side}</td><td style="padding:6px">${o===undefined?'-':o}</td><td style="padding:6px">${n===undefined?'-':n}</td><td style="padding:6px">${net}</td><td style="padding:6px">${mom}</td></tr>`; }); html2+='</tbody></table>'; html2+='<div style="height:8px"></div><strong>AH Lines</strong>'; html2+='<table class="table"><thead><tr><th>Line</th><th>Open H</th><th>Now H</th><th>Net H</th><th>Mom H</th><th>Dir H</th><th>Dir A</th></tr></thead><tbody>'; (r.ah_details||[]).forEach(ad=>{ html2+=`<tr><td style="padding:6px">${ad.line||'-'}</td><td style="padding:6px">${ad.open_home||'-'}</td><td style="padding:6px">${ad.now_home||'-'}</td><td style="padding:6px">${ad.net_home===undefined?'-':nf(ad.net_home,4)}</td><td style="padding:6px">${ad.mom_home===undefined?'-':nf(ad.mom_home,4)}</td><td style="padding:6px">${ad.dir_home||'-'}</td><td style="padding:6px">${ad.dir_away||'-'}</td></tr>`; }); html2+='</tbody></table>'; if(r.metrics&&r.metrics.trapFlags&&r.metrics.trapFlags.length){ html2+=`<div style="margin-top:8px;background:rgba(244,63,94,0.06);border-left:4px solid rgba(244,63,94,0.6);padding:8px;border-radius:8px;color:#ffd6db"><strong>Trap flags:</strong> ${r.metrics.trapFlags.join(', ')}</div>`; } if(r.metrics&&r.metrics.signature&&r.metrics.signature.length){ html2+=`<div style="margin-top:8px;background:rgba(255,140,54,0.04);border-left:4px solid rgba(255,99,72,0.6);padding:8px;border-radius:8px;color:#ffe9c7"><strong>Signature:</strong> ${r.metrics.signature.join(', ')}</div>`; } dt.innerHTML=html2; const stakeEl=document.getElementById('stakeSuggestion'); if(r.stake_pct&&r.stake_pct>0){ stakeEl.innerHTML=`<div style="font-weight:900;color:var(--gold)">${r.stake_pct}%</div><div class="small">แนะนำความระมัดระวัง</div>`; } else { stakeEl.innerHTML=`<div class="small">ไม่แนะนำเดิมพันแบบชัดเจน</div>`; } const tome=document.getElementById('secretTome'); const tc=document.getElementById('tomeContent'); if(r.mother_price||r.intent_price||r.money_price){ tome.style.display='block'; let t=''; if(r.mother_price) t+=`<div><strong>ราคาแม่ (แม่ตั้งตอนเปิด):</strong> ${r.mother_price.line} — เปิด H:${r.mother_price.open_home} / A:${r.mother_price.open_away}</div>`; if(r.intent_price) t+=`<div><strong>ราคาเจตนา (ขยับแรงสุด):</strong> ${r.intent_price.line} — Mom H:${r.intent_price.mom_home} A:${r.intent_price.mom_away}</div>`; if(r.money_price) t+=`<div><strong>ราคาเงินใหญ่ (Momentum มากสุด):</strong> ${r.money_price.line} — Net H:${r.money_price.net_home} A:${r.money_price.net_away}</div>`; t+=`<div style="margin-top:8px;color:#ffdca8">คำแนะนำคัมภีร์: หากราคาแม่ขยับน้อย แต่ราคาเงินใหญ่เปลี่ยนเร็ว → ระวังกับดัก; หากราคาเจตนา+เงินใหญ่สอดคล้อง → เป็นไหลจริง</div>`; tc.innerHTML=t; } else { tome.style.display='none'; tc.innerHTML='ยังไม่มี AH เพียงพอ'; } window.lastAnalysis=r; }
</script>
<script>
// --- FIX: Prevent Mobile Zoom on Input ---
...
// --- CORE: Super-Advanced Market HACK Engine ---
function analyzeOdds() {
    const home = parseFloat(document.getElementById('home_open')?.value || 0);
    const away = parseFloat(document.getElementById('away_open')?.value || 0);
    const homeNow = parseFloat(document.getElementById('home_now')?.value || 0);
    const awayNow = parseFloat(document.getElementById('away_now')?.value || 0);

    const nfHome = home - homeNow;
    const nfAway = away - awayNow;
    const netFlow = Math.abs(nfHome - nfAway).toFixed(3);

    let flowPower = Math.min(100, Math.abs(netFlow * 620)).toFixed(0);
    if (flowPower < 5) flowPower = 5;

    let verdict = "";
    if (netFlow > 0.12) verdict = "ไหลจริง (Sharp Money)";
    else if (netFlow < 0.05) verdict = "ไหลหลอก (Fake Move)";
    else verdict = "ก้ำกึ่ง ต้องระวัง";

    let html = `<div class='oracle-card'>
        <div class='dragon-flare'>🐉 พลังการไหล: <b>${flowPower}/100</b></div>
        <div class='mirror-line'>NetFlow: ${netFlow}</div>
        <div class='verdict'>ผลลัพธ์: <span>${verdict}</span></div>
        <div class='scroll'>คัมภีร์อ่านราคาแม่: จุดเริ่มไหลคือราคาเปิด — ถ้าไหลสวน 1x2 ให้ตีความว่าโต๊ะหลอกทันที</div>
    </div>`;
...
document.getElementById('analyzeBtn')?.addEventListener('click', () => {
    ...
    out.innerHTML = "<div class='oracle-drop'>🐲 มังกรกำลังอ่านคัมภีร์ราคา…</div>";
    setTimeout(() => analyzeOdds(), 800);
});
...
inputs.forEach(i => {
  ...
  i.addEventListener('touchstart', () => document.body.style.zoom = 1);
});
