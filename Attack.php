...
/**
 * Last.php — ALL-IN-ONE: PRE-MATCH MAX + MMM + GHOST + PF + LMM + CTE + ULTRA-SAFE (15-PICK)
 * Single-file deploy (PHP 7.4+) — Uses SQLite via PDO (pdo_sqlite)
 *
 * WARNING: Backup original Last.php and DB before deploying.
 *
 * Endpoints (HTTP GET/POST):
 * - ?action=master_analyze (POST)          => runs integrated analyze (input = JSON payload)
 * - ?action=mark_outcome (POST)            => mark outcome to teach DB (match_key + outcome)
 * - ?action=ewma_stats (GET)               => inspect EWMA store
 * - ?action=match_cases (GET)              => list recent match_cases
 * - ?action=autotune_run (POST)            => run autotune (light)
 * - ?action=backtest (GET)                 => simple backtest on labeled cases
 * - ?action=ultra_safe_picks (POST)        => returns up to N ultra-safe picks for multi-pick ticket
 *
 * DB: last_allin.db
 */
...
// mme_analyze, uds_detect, smkx_score, pce_project reused as defined earlier
function mme_analyze(array $ah_details, array $flow1, ?int $kickoff_ts=null){
    $juice=0.0; foreach ($ah_details as $ad){ $hj = (!is_nan($ad['open_home'] ?? NAN) && !is_nan($ad['now_home'] ?? NAN)) ? abs($ad['open_home']-$ad['now_home']) : 0.0; $aj = (!is_nan($ad['open_away'] ?? NAN) && !is_nan($ad['now_away'] ?? NAN)) ? abs($ad['open_away']-$ad['now_away']) : 0.0; $juice += $hj+$aj; }
...
function probability_fusion(array $features){
    ...
    $score += (1.0 - $fakeflow) * ($w['fakeflow'] ?? 0.10);
    $score += $smk * ($w['smk'] ?? 0.20);
    $score += (1.0 - $div) * ($w['divergence'] ?? 0.10);
...
// ---------------- LMM: Live Micro-Movement Engine ----------------
function lmm_analyze(array $open1, array $now1, array $ah_details, ?int $kickoff_ts=null) {
    $maxRel = 0.0; $sumRel = 0.0; $count = 0;
    foreach (['home','away','draw'] as $s) {
        $o = safeFloat($open1[$s] ?? null); $n = safeFloat($now1[$s] ?? null);
        if (!is_nan($o) && !is_nan($n) && $o>0) {
            $rel = abs(($n - $o) / $o);
            $sumRel += $rel; $count++;
            if ($rel > $maxRel) $maxRel = $rel;
        }
    }
    $avgRel = $count ? ($sumRel / $count) : 0.0;
    $micro_strength = clampf(($avgRel / 0.05), 0.0, 1.0);
    $ah_total_move = 0.0;
    foreach ($ah_details as $ad) {
        $ah_total_move += (!is_nan($ad['open_home'] ?? NAN) && !is_nan($ad['now_home'] ?? NAN)) ? (($ad['open_home'] - $ad['now_home'])) : 0.0;
        $ah_total_move += (!is_nan($ad['open_away'] ?? NAN) && !is_nan($ad['now_away'] ?? NAN)) ? (($ad['open_away'] - $ad['now_away'])) : 0.0;
    }
    $x2_total_move = abs(netflow(safeFloat($open1['home'] ?? null), safeFloat($now1['home'] ?? null))) + abs(netflow(safeFloat($open1['away'] ?? null), safeFloat($now1['away'] ?? null)));
    $pattern = 'neutral';
    if ($micro_strength > 0.12) {
        if ($ah_total_move * 1.0 > $x2_total_move * 1.5) $pattern = 'push';
        elseif ($x2_total_move > $ah_total_move * 1.5) $pattern = 'drain';
    }
    global $pdo;
    if ($pdo && $kickoff_ts) {
        $mk = md5(json_encode($open1).json_encode($now1).microtime(true));
        $st = $pdo->prepare("INSERT INTO samples (k,sample,ts) VALUES(:k,:s,:t)");
        $st->execute([':k'=>'lmm_'.$mk,':s'=>$micro_strength,':t'=>now_ts()]);
    }
    return ['micro_strength'=>$micro_strength,'avgRel'=>$avgRel,'maxRel'=>$maxRel,'pattern'=>$pattern,'ah_total_move'=>$ah_total_move,'x2_total_move'=>$x2_total_move];
}

// ---------------- Kelly sizing ----------------
function kelly_fraction(float $edge, float $odds, float $fraction=0.5){
    if ($odds <= 1.0) return 0.0;
    $b = $odds - 1.0;
    $k = ($edge * (1.0 + $b) - (1.0 - $edge)) / $b;
    $k = clampf($k, 0.0, 1.0);
    return $k * $fraction;
}
function stake_suggestion_from_pf(array $features, array $true_prob, float $pf_score, float $confidence){
    $implied_home = (isset($features['implied_home']) ? $features['implied_home'] : (1.0 / ($features['now_home'] ?? 2.0)));
    $implied_away = (isset($features['implied_away']) ? $features['implied_away'] : (1.0 / ($features['now_away'] ?? 2.0)));
    $edges = [];
    if (isset($true_prob['home'])) $edges['home'] = $true_prob['home'] - $implied_home;
    if (isset($true_prob['away'])) $edges['away'] = $true_prob['away'] - $implied_away;
    arsort($edges);
    $top = key($edges);
    $edge = max(0.0, $edges[$top] ?? 0.0);
    $odds = ($top === 'home') ? ($features['now_home'] ?? 2.0) : ($features['now_away'] ?? 2.0);
    $kel = kelly_fraction($edge, floatval($odds), 0.5);
    $unit = round($kel * 100, 2);
    return ['side'=>$top,'edge'=>$edge,'kelly_fraction'=>$kel,'suggest_pct_of_bankroll'=>$unit];
}

// ---------------- Autotune (light) ----------------
function autotune_run(){
    global $pdo, $CONFIG;
    if (!$pdo) return ['status'=>'error','msg'=>'no_db'];
    $minCases = 30;
    $st = $pdo->query("SELECT COUNT(*) FROM match_cases WHERE outcome != ''");
    $c = intval($st->fetchColumn());
    if ($c < $minCases) return ['status'=>'skipped','reason'=>'not_enough_outcomes','cases'=>$c];
    $K = min(200, $c);
    $q = $pdo->prepare("SELECT id,analysis,outcome FROM match_cases WHERE outcome != '' ORDER BY id DESC LIMIT :k");
    $q->bindValue(':k', $K, PDO::PARAM_INT); $q->execute();
    $rows = $q->fetchAll(PDO::FETCH_ASSOC);
    $current_weights = $CONFIG['pf_weights'];
    $st2 = $pdo->query("SELECT signature,win_rate,count FROM pattern_memory ORDER BY count DESC LIMIT 200");
    $pats = $st2->fetchAll(PDO::FETCH_ASSOC);
    $avgWR = 0.0; $pc=0; foreach($pats as $p){ $avgWR += floatval($p['win_rate']); $pc++; }
    if ($pc>0) $avgWR /= $pc;
    $delta = 0.0;
    if ($avgWR > 0.56) $delta = 0.005;
    elseif ($avgWR < 0.48) $delta = -0.005;
    else $delta = 0.0;
    $keys = array_keys($CONFIG['pf_weights']);
    foreach($keys as $k){
        if ($k === 'smk' || $k === 'netflow') $new = clampf($CONFIG['pf_weights'][$k] + $delta, 0.01, 0.5);
        else $new = clampf($CONFIG['pf_weights'][$k] - ($delta*0.2), 0.01, 0.5);
        ewma_update('pf_w_'.$k, $new);
    }
    if ($pdo) {
        $lg = $pdo->prepare("INSERT INTO autotune_log (param,old,new,reason,ts) VALUES(:p,:o,:n,:r,:t)");
        $lg->execute([':p'=>'pf_weights_autotune',':o'=>$avgWR,':n'=>json_encode($CONFIG['pf_weights']),':r'=>'autotune_by_pattern_avgWR_'.$avgWR,':t'=>now_ts()]);
    }
    return ['status'=>'ok','cases'=>$c,'avgWR'=>$avgWR];
}

// ---------------- Backtest light ----------------
function backtest_recent_cases(int $limit=200){
    global $pdo;
    if (!$pdo) return ['status'=>'error','msg'=>'no_db'];
    $st = $pdo->prepare("SELECT id,analysis,outcome FROM match_cases WHERE outcome != '' ORDER BY id DESC LIMIT :l");
    $st->bindValue(':l',$limit,PDO::PARAM_INT); $st->execute();
    $rows = $st->fetchAll(PDO::FETCH_ASSOC);
    $wins=0;$total=0;$sumEdge=0.0;
    foreach($rows as $r){
        $a = json_decode($r['analysis'], true);
        if (!$a || empty($a['true_prob'])) continue;
        $pred = ($a['true_prob']['home'] > $a['true_prob']['away']) ? 'home' : 'away';
        $total++;
        if ($pred === $r['outcome']) $wins++;
    }
    $wr = $total ? ($wins/$total) : 0.0;
    return ['status'=>'ok','total'=>$total,'wins'=>$wins,'winrate'=>round($wr,4)];
}
...
// prematch_integrity_check and early_drop_detector
...
// ---------------- PM-MAX COMPLETE (master_analyze) ----------------
function master_analyze(array $payload){
    ...
    $lmm = lmm_analyze($open1,$now1,$ah_details,$kickoff);
    ...
    $features_pf = [
        ...
        'lmm_strength'=>clampf($lmm['micro_strength'],0,1)
    ];
    ...
    return [
        ...
        'metrics'=>[
            ...
            'pf'=>$pf,'mmm'=>$mmm_res,'ghost'=>$ghost,'lmm'=>$lmm,'features_pf'=>$features_pf
        ],
...
function pm_learn($signature, bool $win, array $meta=[]){
    ...
    if ($r) {
        ...
        $prev_wr = floatval($r['win_rate']);
        $new_wr = (($prev_wr * intval($r['count'])) + ($win?1.0:0.0)) / max(1,$count);
        $upd = $pdo->prepare("UPDATE pattern_memory SET count=:c,win_rate=:w,last_seen=:ls WHERE id=:id");
        $upd->execute([':c'=>$count,':w'=>$new_wr,':ls'=>$now,':id'=>$r['id']]);
    } else {
        $ins = $pdo->prepare("INSERT INTO pattern_memory (signature,count,win_rate,last_seen,meta) VALUES(:s,1,:wr,:ls,:m)");
        $ins->execute([':s'=>$sig,':wr'=>($win?1.0:0.0),':ls'=>$now,':m'=>json_encode($meta)]);
    }
    return true;
}

if (php_sapi_name() !== 'cli') {
    // master_analyze endpoint
    ...
    if (isset($_GET['action']) && $_GET['action']==='autotune_run' && $_SERVER['REQUEST_METHOD']==='POST') {
        $res = autotune_run();
        header('Content-Type: application/json; charset=utf-8'); echo json_encode($res, JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT); exit;
    }
    if (isset($_GET['action']) && $_GET['action']==='backtest' && $_SERVER['REQUEST_METHOD']==='GET') {
        $l = isset($_GET['limit']) ? intval($_GET['limit']) : 200;
        $res = backtest_recent_cases($l);
        header('Content-Type: application/json; charset=utf-8'); echo json_encode($res, JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT); exit;
    }
}

// ---------------- ULTRA SAFE 15-PICK ENGINE ----------------
function ultra_safe_score_single(array $case) {
    $open1 = $case['open1'] ?? ['home'=>NAN,'draw'=>NAN,'away'=>NAN];
    $now1  = $case['now1']  ?? ['home'=>NAN,'draw'=>NAN,'away'=>NAN];
    $ah = $case['ah'] ?? [];
    $pmmax = prematch_max_analyze($open1,$now1,$ah);
    $tpo = tpo_estimate($now1)['tpo'];
    $mme = mme_analyze(array_map(function($x){ return $x;}, $ah), ['home'=>netflow(safeFloat($open1['home']??null),safeFloat($now1['home']??null)),'away'=>netflow(safeFloat($open1['away']??null),safeFloat($now1['away']??null))], isset($case['kickoff'])?intval(strtotime($case['kickoff'])):null);
    $uds = uds_detect(array_map(function($x){ return $x;}, $ah), ['home'=>netflow(safeFloat($open1['home']??null),safeFloat($now1['home']??null)),'away'=>netflow(safeFloat($open1['away']??null),safeFloat($now1['away']??null))]);
    $smkx = smkx_score($tpo,$pmmax['decon']['true_prob'],$mme,$uds);
    $ghost = ghost_layer($open1,$now1, isset($case['kickoff']) ? intval(strtotime($case['kickoff'])) : null);
    $lmm  = function_exists('lmm_analyze') ? lmm_analyze($open1,$now1,$ah, isset($case['kickoff'])?intval(strtotime($case['kickoff'])):null) : ['micro_strength'=>0,'pattern'=>'neutral'];
    $pf_features = [
        'to_gap'=>clampf(abs((safeFloat($open1['home']??NAN)-safeFloat($now1['home']??NAN))/max(1,safeFloat($open1['home']??1))),0,1),
        'netflow'=>clampf(abs(netflow(safeFloat($open1['home']??NAN),safeFloat($now1['home']??NAN)) - netflow(safeFloat($open1['away']??NAN),safeFloat($now1['away']??NAN))),0,1),
        'fakeflow'=>clampf($pmmax['bid']['score'] ?? 0,0,1),
        'smk'=>clampf($smkx['score'] ?? 0,0,1),
        'divergence'=>clampf(($uds['divergence'] ?? 0)/1.0,0,1),
        'mmm_winrate'=>0.0,
        'ghost'=>clampf(($ghost['ghost_strength'] ?? 0)/100.0,0,1),
        'ewma'=>clampf(ewma_get('master_voidscore',0.0)['v'],0,1)
    ];
    $pf = probability_fusion($pf_features);
    $mmm = mmm_lookup($pmmax['decon'],$pmmax['bid'],$ah);
    $checks = [];
    $checks[] = ['ok'=>($pf['score'] >= 0.88),'reason'=>'pf_score_too_low','value'=>$pf['score'],'need'=>0.88];
    $checks[] = ['ok'=>($pmmax['score'] < 0.6),'reason'=>'pmmax_too_high','value'=>$pmmax['score'],'need'=>'<0.6'];
    $checks[] = ['ok'=>($smkx['score'] >= 0.7),'reason'=>'smk_low','value'=>$smkx['score'],'need'=>0.7];
    $checks[] = ['ok'=>($ghost['ghost_strength'] <= 20),'reason'=>'ghost_too_high','value'=>$ghost['ghost_strength'],'need'=>20];
    $checks[] = ['ok'=>($lmm['micro_strength'] <= 0.12),'reason'=>'micro_move_high','value'=>$lmm['micro_strength'],'need'=>0.12];
    $checks[] = ['ok'=>(!$pmmax['bid']['type'] || $pmmax['bid']['type']!=='deceptive'),'reason'=>'book_deceptive','value'=>($pmmax['bid']['type'] ?? 'none'),'need'=>'not_deceptive'];
    $mmm_ok = false;
    if (($mmm['matches'] ?? 0) >= 7) $mmm_ok = true;
    elseif (($mmm['matches'] ?? 0) >= 4 && ($mmm['win_rate'] ?? 0) >= 0.78) $mmm_ok = true;
    $checks[] = ['ok'=>$mmm_ok,'reason'=>'mmm_not_confident','value'=>['matches'=>$mmm['matches'] ?? 0,'wr'=>$mmm['win_rate'] ?? 0],'need'=>'matches>=7 || (matches>=4 & wr>=0.78)'];
    $void_strength = isset($pmmax['score']) ? $pmmax['score'] : 0;
    $checks[] = ['ok'=>($void_strength <= 0.35),'reason'=>'void_too_high','value'=>$void_strength,'need'=>0.35];
    $confidence = round(clampf( 70 + ($pf['score']*20) + ($smkx['score']*5) + ((1.0 - ($ghost['ghost_strength']/100.0))*5) + ((1.0 - $lmm['micro_strength'])*3), 0, 100),2);
    $passed = true;
    $failed_reasons = [];
    foreach ($checks as $c) { if (!$c['ok']) { $passed=false; $failed_reasons[]=$c; } }
    if ($confidence < 96.5) { $passed=false; $failed_reasons[]=['ok'=>false,'reason'=>'confidence_too_low','value'=>$confidence,'need'=>96.5]; }
    $scoreComposite = ($pf['score']*0.5) + ($smkx['score']*0.3) + ((1.0 - ($ghost['ghost_strength']/100.0))*0.1) + ((1.0 - $lmm['micro_strength'])*0.1);
    return [
        'case'=>$case,'pf'=>$pf,'pmmax'=>$pmmax,'smkx'=>$smkx,'ghost'=>$ghost,'lmm'=>$lmm,'mmm'=>$mmm,
        'checks'=>$checks,'passed'=>$passed,'failed_reasons'=>$failed_reasons,'confidence'=>$confidence,'composite'=>$scoreComposite
    ];
}

function ultra_safe_select(array $cases, int $limit=15) {
    $scored = [];
    foreach ($cases as $c) {
        $r = ultra_safe_score_single($c);
        $scored[] = $r;
    }
    $passed = array_filter($scored, function($x){ return isset($x['passed']) && $x['passed']===true; });
    usort($passed, function($a,$b){
        $ca = $a['composite'] ?? 0; $cb = $b['composite'] ?? 0;
        if ($ca==$cb) {
            $ca2 = $a['confidence'] ?? 0; $cb2 = $b['confidence'] ?? 0;
            if ($ca2==$cb2) {
                $am = $a['mmm']['matches'] ?? 0; $bm = $b['mmm']['matches'] ?? 0;
                return $bm - $am;
            }
            return ($cb2 < $ca2) ? 1 : -1;
        }
    ...
    $selected = array_slice($passed, 0, $limit);
    $rejected = array_filter($scored, function($x){ return !($x['passed'] ?? false); });
    $outSel = array_map(function($s){ return [
        'home'=>$s['case']['home'] ?? null,'away'=>$s['case']['away'] ?? null,'league'=>$s['case']['league'] ?? null,
        'confidence'=>$s['confidence'],'composite'=>$s['composite'],'pf'=>$s['pf'],'smkx'=>$s['smkx'],'mmm'=>$s['mmm']
    ]; }, $selected);
    $outRej = array_map(function($s){ return [
        'home'=>$s['case']['home'] ?? null,'away'=>$s['case']['away'] ?? null,'league'=>$s['case']['league'] ?? null,
        'confidence'=>$s['confidence'],'failed_reasons'=>$s['failed_reasons']
    ]; }, $rejected);
    return ['selected'=>$outSel,'rejected'=>$outRej,'total_candidates'=>count($cases),'selected_count'=>count($outSel)];
}

if (php_sapi_name() !== 'cli') {
    if (isset($_GET['action']) && $_GET['action']==='ultra_safe_picks' && $_SERVER['REQUEST_METHOD']==='POST') {
        $raw = file_get_contents('php://input'); $p = json_decode($raw,true);
        if (!is_array($p) || !isset($p['matches'])) { header('Content-Type: application/json; charset=utf-8'); echo json_encode(['status'=>'error','msg'=>'invalid_payload'], JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT); exit; }
        $limit = isset($p['limit']) ? intval($p['limit']) : 15;
        $res = ultra_safe_select($p['matches'],$limit);
        header('Content-Type: application/json; charset=utf-8'); echo json_encode(array_merge(['status'=>'ok'], $res), JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT); exit;
    }
}

// ---------------- Minimal UI for quick test (optional) ----------------
if (php_sapi_name() !== 'cli') {
?><!doctype html>
<html lang="th">
<head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Last.php — All-in-One</title>
<style>
...
<body>
<div class="container">
  <div class="card"><h2 style="margin:0">PRE-MATCH MAX — FULL (MMM + Ghost + PF + LMM + ULTRA-SAFE)</h2><div class="small">Integrated system — Ready to analyze</div></div>
  ...
  <div class="card">
    <h3>Ultra Safe Picks Tester</h3>
    <div class="small">วาง JSON matches (array) แล้วกดส่ง</div>
    <textarea id="matchesJSON" style="width:100%;height:160px;background:#000;color:#fff;padding:8px;border-radius:8px"></textarea>
    <div style="height:8px"></div>
    <div style="text-align:right"><button id="ultraBtn" class="btn">🔰 Ultra Safe Picks</button></div>
    <div id="ultraResult" style="margin-top:8px;color:#d9c89a"></div>
  </div>
</div>
...
// Ultra Safe UI
document.getElementById('ultraBtn').addEventListener('click', async ()=>{
  const txt = document.getElementById('matchesJSON').value;
  let arr;
  try { arr = JSON.parse(txt); if (!Array.isArray(arr)) throw new Error('ต้องเป็น array'); } catch(e){ document.getElementById('ultraResult').innerText = 'JSON parse error: '+e.message; return; }
  document.getElementById('ultraResult').innerText = 'Running...';
  try {
    const res = await fetch('?action=ultra_safe_picks', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({matches:arr,limit:15})});
    const j = await res.json(); document.getElementById('ultraResult').innerText = JSON.stringify(j, null, 2);
  } catch (e) { document.getElementById('ultraResult').innerText = 'Error: '+e.message; }
});
</script>
</body>
</html>
