...
/*
 Last.php — VOID ENGINE v7 — Market Breaker Edition
 - Backup original Last.php as Last.php.bak.YYYYmmdd_HHMMSS (script will attempt to make a backup if overwriting)
 - Modules included:
    • EWMA adaptive store (SQLite)
    ...
    • OUCE (OU Correlation Engine, inferred)
 - API endpoint: ?action=void_analyze (POST JSON)
 - EWMA debug: ?action=ewma_stats
 - UI included (Chinese fantasy theme) — responsive
*/
...
$basedir = __DIR__;
$last_path = $basedir . '/Last.php'; // placeholder for backup logic if desired

// If user explicitly wants auto-backup when saving new file, keep in mind to copy old file beforehand.
// (This file as provided won't overwrite automatically — deployment is manual by copying to server)
...
// ---------------- LVE: Liquidity Velocity Engine ----------------
// uses historical samples for 'void_netflow' to estimate velocity/acceleration
...
function lve_compute(float $current_sample): array {
    // returns velocity, acceleration, jerk, money_weight (0-100)
    $rows = get_last_samples('void_netflow', 6); // includes current stored previously
    // append current sample with now timestamp for delta calc (no DB write here)
    $now_ts = time();
    $data = [];
    ...
    // money_weight heuristic
    $mw = min(1.0, abs($velocity) * 200.0 + max(0.0, $acceleration)*50.0); // heuristic
...
function sfd_analyze(float $current_sample): array {
    // uses last samples of void_netflow to detect jumps/spoof patterns
    ...
    if (count($vals) >= 3) {
        ...
        // delayed reflect heuristic: if AH moved earlier but no 1x2 follow-up -> external detection placed elsewhere (SFD marks increased spoof prob)
...
// ---------------- OU Correlation Engine (inferred, no external OU) ----------------
function ouce_infer(array $ah_details, array $flow1): array {
    // infer OU misalignment from AH and 1x2 movement patterns
    $score=0.0; $notes=[];
    // if AH heavy move to favor home but 1x2 draw/away unchanged -> misalign
    ...
    // infer OU mismatch heuristics
    // high score -> OU likely out of sync -> increase spoof risk
    $score = clampf($score, 0.0, 1.0);
    $confidence_drop = $score * 0.4; // fraction to lower confidence if mismatch
...
function void_engine_analyze(array $payload): array {
    ...
    // LVE: use agg (simple aggregator) as current sample for liquidity engine
    $agg_sample = 0.0;
    ...
    $liquidity_weight = $lve['money_weight']; // 0..100

    // SFD
    $sfd = sfd_analyze(floatval($agg_sample));
    $spike_penalty = $sfd['spike_score']; // 0..100

    // OUCE
    $ouce = ouce_infer($ah_details, $flow1);
    $ou_mismatch = $ouce['ou_mismatch_score']; // 0..1
    ...
    return [
        ...
        'metrics'=>array_merge($metrics_for_void,['marketMomentum'=>$marketMomentum,'total1x2_mom'=>$total1x2_mom,'totalAH_mom'=>$totalAH_mom,'reboundSens'=>$reboundSens,'flowPower'=>$flowPower,'confidence'=>$confidence,'signature'=>$signature,'trapFlags'=>$trapFlags,'fda'=>$fda,'smk_flags'=>$smartFlags,'lve'=>$lve,'sfd'=>$sfd,'ouce'=>$ouce]),
...
// ---------------- UI (same theme as before) ----------------
?><!doctype html>
<html lang="th">
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>THE VOID — Market Breaker Edition (v7)</title>
<style>
  ...
  <div class="container">
    <div class="header">
      <div style="display:flex;align-items:center;gap:12px">
        ...
        <div>
          <h1 style="margin:0;color:var(--gold)">THE VOID — Market Breaker (v7)</h1>
          <div style="color:#d9c89a;font-size:0.95rem">SMK3.0 • FDA • LVE • SFD • OUCE • EWMA learning (SQLite)</div>
        </div>
      </div>
...
<script>
// JS UI same as previous versions — collect input, POST to ?action=void_analyze
