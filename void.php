...
/*
  Last_void.php — THE VOID ENGINE (Level ∞)
  - Backup Last.php if exists
  - Include Last.php to preserve original logic
  - SQLite-backed EWMA & learning store
  - Advanced engine: SMK, NetFlow, Divergence, Rebound auto, VOID compression
  - API endpoint: ?action=void_analyze (POST JSON)
  - UI: Chinese fantasy + VOID mode (responsive)
  Usage:
    1) place this file alongside Last.php
    2) open Last_void.php in browser
    3) test analyze via UI
*/
...
$basedir = __DIR__;
$last_path = $basedir . 'index.php';

// auto-backup Last.php (only if present)
if (file_exists($last_path)) {
    $bak = $basedir . '/index.php.bak.' . date('Ymd_His');
    if (!file_exists($bak)) {
        @copy($last_path, $bak);
    }
}

// include original to keep functions/definitions (if safe)
if (file_exists($last_path)) {
    // include in isolated scope
    try {
        require_once $last_path;
    } catch (\Throwable $e) {
        // don't break — continue with our wrapper
...
function ewma_update(string $k, float $sample) {
    ...
    // store sample history for adaptive alpha tuning
...
// Void compression: compress many signals into single truth score (THE VOID RESULT)
function void_compress_score(array $metrics): float {
    // metrics expected keys: flowPower, confidence, smartMoneyScore, divergence, juicePressureNorm, stackFactor, trap
    // compress via weighted robust aggregator
    $w = [
        'flowPower' => 0.28,
        'confidence' => 0.22,
        ...
        'stackFactor' => 0.10,
        'divergence' => -0.1 // divergence reduces raw truth (high divergence indicates conflict)
    ];
    $score = 0.0;
    foreach ($w as $k => $wt) {
        $val = isset($metrics[$k]) ? floatval($metrics[$k]) : 0.0;
        // normalize common fields 0..100 to 0..1
        if (in_array($k, ['flowPower','confidence'])) $val = $val/100.0;
        if ($k === 'divergence') { $val = clampf($val/0.3, 0.0, 1.0); } // 0..0.3 normalized
        $score += $wt * $val;
    }
    // trap flag strongly reduces score
    if (!empty($metrics['trap'])) $score *= 0.28;
    // clamp -1..1 range
    return clampf($score, -1.0, 1.0);
}

// Main merged analyze function (void-aware)
function void_engine_analyze(array $payload): array {
    ...
    if ($divergence > 0.12) { $smartMoneyScore += 0.15; $smartFlags[]='divergence'; }
    ...
    $dirNorm = 0.0;
    if ($rawSignal > 1e-6) $dirNorm = tanh($dirScore / (0.5 + $marketMomentum));
    ...
    $void_score = void_compress_score($metrics_for_void); // -1..1

    // translate void_score to final label
    if ($market_kill) {
        $final_label = '💀 MARKET KILL — ห้ามเสี่ยง';
        $recommendation = 'ห้ามเสี่ยง/ควบคุมเดิมพันอย่างเคร่งครัด';
    } elseif ($void_score > 0.35) {
        $final_label = '✅ VOID: ไหลจริง — ฝั่งเหย้า (support)';
        $recommendation = 'พิจารณาตาม ไม่เกินความเสี่ยงแนะนำ';
    } elseif ($void_score < -0.35) {
        $final_label = '✅ VOID: ไหลจริง — ฝั่งเยือน (attack)';
        $recommendation = 'พิจารณาตาม ไม่เกินความเสี่ยงแนะนำ';
    } elseif ($trap) {
    ...
    return [
        ...
        'metrics'=>array_merge($metrics_for_void,['marketMomentum'=>$marketMomentum,'total1x2_mom'=>$total1x2_mom,'totalAH_mom'=>$totalAH_mom,'reboundSens'=>$reboundSens,'flowPower'=>$flowPower,'confidence'=>$confidence,'signature'=>$signature,'trapFlags'=>$trapFlags]),
...
if (php_sapi_name() !== 'cli') {
    // POST JSON to ?action=void_analyze
...
<title>THE VOID — Imperial Odds Analyzer (Level ∞)</title>
<style>
:root{
  --gold:#d4a017; --royal:#5b21b6; --card:#0f0b0a; --muted:#d9c89a; --danger:#ff3b30;
  --jade1:#7ee2c7; --jade2:#1aa07a; --ruby:#b00010; --void:#0b0b0f;
}
*{box-sizing:border-box}
body{margin:0;font-family:Inter, 'Noto Sans Thai', Arial;background:linear-gradient(180deg,#030308,#0a0710);color:#f6eedf; -webkit-font-smoothing:antialiased}
.container{max-width:1200px;margin:18px auto;padding:18px}
.header{display:flex;align-items:center;justify-content:space-between;gap:12px}
.logo{width:92px;height:92px;border-radius:14px;background:radial-gradient(circle at 30% 20%, rgba(255,255,255,0.04), rgba(91,33,182,0.95));display:flex;align-items:center;justify-content:center;color:white;font-weight:900;font-size:36px;box-shadow:0 12px 40px rgba(0,0,0,0.6)}
  ...
  <div class="container">
    <div class="header">
      <div style="display:flex;align-items:center;gap:12px">
        ...
        <div>
          <h1 style="margin:0;color:var(--gold)">THE VOID — Imperial Odds Analyzer</h1>
          <div style="color:#d9c89a;font-size:0.95rem">Level ∞ — Void Engine (SQLite learning, SMK, NetFlow, Divergence, Void compress)</div>
        </div>
      </div>
    ...
    <div id="resultWrap" class="card" style="display:none;position:relative;overflow:visible">
      ...
      <div class="resultWrap">
        <div class="analysisCard">
          ...
          <div id="tome" class="tome" aria-hidden="true"></div>
        </div>
...
function renderResult(r){
  ...
  ['home','draw','away'].forEach(side=>{
    ...
    html2+=`<tr><td>${side}</td><td>${o}</td><td>${n}</td><td>-</td><td>-</td></tr>`;
  });
  html2+='</tbody></table>';
